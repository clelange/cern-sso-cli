...
func TestOTPProvider_GetMaxRetries_ZeroDisablesRetry(t *testing.T) {
	p := &OTPProvider{MaxRetries: 0, DisableRetries: true}
	if p.GetMaxRetries() != 1 {
		t.Errorf("GetMaxRetries() = %d, want 1 (single attempt when retries are disabled)", p.GetMaxRetries())
	}
}

func TestOTPProvider_GetMaxRetries_NegativeUsesDefault(t *testing.T) {
	p := &OTPProvider{MaxRetries: -1}
	if p.GetMaxRetries() != 3 {
		t.Errorf("GetMaxRetries() = %d, want 3 (default for negative values)", p.GetMaxRetries())
	}
}

func TestIsAutoGeneratedTOTPSource(t *testing.T) {
	t.Setenv(EnvOTPCommand, "echo 123456")

	if !isAutoGeneratedTOTPSource(OTPSourceCommand) {
		t.Error("OTPSourceCommand should be treated as an auto-generated TOTP source")
	}
	if !isAutoGeneratedTOTPSource(OTPSourceKeychain) {
		t.Error("OTPSourceKeychain should be treated as an auto-generated TOTP source")
	}
	if !isAutoGeneratedTOTPSource(OTPSourceEnv) {
		t.Error("OTPSourceEnv with CERN_SSO_OTP_COMMAND set should be treated as an auto-generated TOTP source")
	}

	t.Setenv(EnvOTPCommand, "")
	if isAutoGeneratedTOTPSource(OTPSourceEnv) {
		t.Error("OTPSourceEnv without CERN_SSO_OTP_COMMAND should not be treated as an auto-generated TOTP source")
	}
}

func TestMaxOTPAttemptsForSource(t *testing.T) {
	t.Setenv(EnvOTPCommand, "echo 123456")

	if attempts := maxOTPAttemptsForSource(OTPSourceCommand, 3); attempts != 2 {
		t.Errorf("maxOTPAttemptsForSource(command, 3) = %d, want 2", attempts)
	}
	if attempts := maxOTPAttemptsForSource(OTPSourceKeychain, 5); attempts != 2 {
		t.Errorf("maxOTPAttemptsForSource(keychain, 5) = %d, want 2", attempts)
	}
	if attempts := maxOTPAttemptsForSource(OTPSourceEnv, 4); attempts != 2 {
		t.Errorf("maxOTPAttemptsForSource(env-command, 4) = %d, want 2", attempts)
	}
	if attempts := maxOTPAttemptsForSource(OTPSourcePrompt, 5); attempts != 5 {
		t.Errorf("maxOTPAttemptsForSource(prompt, 5) = %d, want 5", attempts)
	}
}
